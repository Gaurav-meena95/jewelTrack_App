...
const createOrders = async (req, res) => {
    try {
        ...
        console.log('[Orders] Created — ID:', newOrder._id, '| customer:', phone, '| total:', total)
        return res.status(201).json({ success: true, message: 'Order created successfully', data: { order: populated } })
    } catch (error) {
        console.log('[Orders] createOrders ERROR —', error)
...
const allOrders = async (req, res) => {
    try {
        ...
        console.log('[Orders] allOrders ERROR —', error)
        return res.status(500).json({ success: false, message: 'Internal Server Error' })
    }
}
...
const updateOrders = async (req, res) => {
    try {
        ...
        console.log('[Orders] Updated — ID:', order_id, '| status:', orderStatus)
        return res.status(200).json({ success: true, message: 'Order updated successfully', data: { order: updated } })
    } catch (error) {
        console.log('[Orders] updateOrders ERROR —', error)
        return res.status(500).json({ success: false, message: 'Internal Server Error' })
    }
}
...
const deleteOrders = async (req, res) => {
    try {
        ...
        console.log('[Orders] Deleted — ID:', order_id)
        return res.status(200).json({ success: true, message: 'Order deleted successfully' })
    } catch (error) {
        console.log('[Orders] deleteOrders ERROR —', error)
        return res.status(500).json({ success: false, message: 'Internal Server Error' })
    }
}
